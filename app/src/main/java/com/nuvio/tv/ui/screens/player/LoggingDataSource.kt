...
class LoggingDataSource(
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        return try {
            ...
            val line = "[$site] pos=${dataSpec.position} reqLen=${dataSpec.length} -> len=$len ms=$elapsed"
            Log.i("DS_OPEN", line)
            recordEvent(line)
            len
        } catch (e: Exception) {
            val elapsed = SystemClock.elapsedRealtime() - t0
            val line = "[$site] pos=${dataSpec.position} FAILED ${e.javaClass.simpleName}: ${e.message} ms=$elapsed"
            Log.w("DS_OPEN", line)
            recordEvent(line)
            throw e
        }
