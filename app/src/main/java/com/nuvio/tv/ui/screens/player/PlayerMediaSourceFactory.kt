...
internal class PlayerMediaSourceFactory {
    ...
    companion object {
            ...
            return try {
                probeHttpClient.newCall(buildProbeRequest(url = url, headers = headers, method = "HEAD"))
                    ...
                    .use { response ->
                        ...
                        val responseHeaders = readResponseHeaders(response)
        ...
        private fun readResponseHeaders(response: Response): Map<String, String> {
            return buildMap {
                response.headers.names().forEach { key ->
                    val value = response.header(key)?.trim().orEmpty()
                    if (value.isNotEmpty()) {
                        put(key, value)
                    }
                }
        ...
        fun extractUserInfoAuth(
            ...
            // Already has an Authorization header; do not override.
